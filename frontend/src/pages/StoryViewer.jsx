...
export default function StoryViewer({ stories, setStories }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (story) setStories(prev => prev.map(s => s.id===id ? {...s, seen:true} : s));
  }, [id]);

  if (!story) return <div className="grid place-items-center h-[60vh] text-white">Story not found or expired.</div>;
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 text-white max-w-md mx-auto w-full">
        <div className="flex items-center gap-2">
          <img src={story.avatar} className="w-8 h-8 rounded-full" />
      ...
      <div className="absolute inset-0 flex">
        ...
        <button className="flex-1" onClick={() => { const nxt = stories[idx+1]; if(nxt) navigate(`/stories/${nxt.id}`, {replace:true}); else navigate("/"); }} aria-label="next" />
      </div>
    </div>
  );
}
