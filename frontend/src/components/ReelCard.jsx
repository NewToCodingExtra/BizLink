...
export default function ReelCard({ opp, onToggleLike, onInquire, onOpenComments }) {
  ...
  return (
    <div className="snap-start relative h-[100dvh] w-full bg-black flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 flex items-end justify-between gap-4 max-w-md mx-auto w-full">
        <div className="flex-1 min-w-0">
          <p className="text-white font-semibold text-sm flex items-center gap-2">
            ...
            <span className="text-[10px] tracking-widest bg-surface/20 px-2 py-0.5 rounded-full shrink-0">{opp.type}</span>
          </p>
        ...
        <div className="flex flex-col items-center gap-3">
          <button onClick={()=>onToggleLike(opp.id)} aria-label="Like" className={`w-10 h-10 rounded-full grid place-items-center text-lg ${opp.liked ? "bg-red-500 text-white" : "bg-surface/20 text-white"}`}>{opp.liked?"♥":"♡"}</button>
          <button onClick={()=>onOpenComments && onOpenComments(opp)} aria-label="Comments" className="w-10 h-10 rounded-full grid place-items-center bg-surface/20 text-white text-sm font-semibold">💬{commentCount > 0 ? <span className="text-[10px] ml-0.5">{commentCount}</span> : null}</button>
          ...
          <button onClick={()=>setMuted(!muted)} aria-label={muted ? "Unmute" : "Mute"} className="w-8 h-8 rounded-full bg-surface/20 text-white grid place-items-center text-xs">{muted?"🔇":"🔊"}</button>
          <div className="relative">
            <button onClick={() => {
              const el = document.getElementById(`menu-reel-${opp.id}`);
              el.classList.toggle('hidden');
            }} className="w-8 h-8 rounded-full bg-surface/20 text-white grid place-items-center">
      ...
      <span className="absolute top-3 left-1/2 -translate-x-1/2 text-[11px] tracking-widest text-white/70 bg-surface/10 px-2 py-1 rounded-full">TAP TO {muted?"UNMUTE":"MUTE"}</span>
    </div>
  );
}
