    ...
    <form onSubmit={submit} className={compact ? "p-1" : "bg-surface rounded-xl border border-border shadow-sm p-6 md:p-8"}>
      ...
      <div className="mt-5 space-y-4">
        <div>
          ...
          <input value={name} onChange={e=>setName(e.target.value)} placeholder="Juan Dela Cruz" className="mt-1 w-full border border-border focus:border-[#2563EB] focus:ring-2 focus:ring-blue-100 outline-none rounded-lg px-3 py-2.5 text-sm placeholder:text-text-secondary" />
        </div>
        <div>
          <label className="text-sm font-medium text-text-primary">Email</label>
          <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="juan@email.com" className="mt-1 w-full border border-border focus:border-[#2563EB] focus:ring-2 focus:ring-blue-100 outline-none rounded-lg px-3 py-2.5 text-sm placeholder:text-text-secondary" />
        </div>
        <div>
          <label className="text-sm font-medium text-text-primary">Message</label>
          <textarea value={message} onChange={e=>setMessage(e.target.value)} maxLength={500} rows={4} placeholder="Tell us what you need..." className="mt-1 w-full border border-border focus:border-[#2563EB] focus:ring-2 focus:ring-blue-100 outline-none rounded-lg px-3 py-2.5 text-sm placeholder:text-text-secondary resize-none" />
          <p className="text-xs text-text-secondary mt-1">{message.length}/500</p>
        </div>
